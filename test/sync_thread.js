console.time("sync_thread_all_the_work")
...
asyn.sync_thread(times,i,function(err, result){
	console.log('fibo('+i+'):' + result)
	console.log('c++ thread sync thread callback')
	assert.strictEqual(result, 102334155)
	if(!--job_times){
		console.timeEnd("sync_thread_all_the_work")
		global.cb();
	}
})
